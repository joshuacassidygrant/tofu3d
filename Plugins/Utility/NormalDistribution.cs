...
namespace TofuPlugin.Utility {
    public class NormalDistribution {

        public double StandardDeviation;
        public double Variance => Math.Pow(StandardDeviation, 2);
        public double Mean;
        public double MaxDeviations;
        public double Max => Mean + MaxDeviations * StandardDeviation;
        public double Min => Mean - MaxDeviations * StandardDeviation;

        public NormalDistribution(double standardDeviation, double mean, double maxDeviations = 4) {
            StandardDeviation = standardDeviation;
            Mean = mean;
            MaxDeviations = maxDeviations;
        }

        public double SampleAt(double point) {
            bool pos = UnityEngine.Random.Range(0, 2) == 1;
            double val = QNorm(Math.Abs(point), Mean, StandardDeviation, !pos, false);
            return val;
        }

        public double Sample() {
            double pt = UnityEngine.Random.Range(0f, 1f);
            return SampleAt(pt);
        }

        public int SampleInt() {
            return (int)Mathf.Clamp(Mathf.CeilToInt((float)Sample()), (float)Mean - (float)Variance, (float)Mean + (float)Variance);
        }
